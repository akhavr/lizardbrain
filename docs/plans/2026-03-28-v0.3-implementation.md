# clawmem v0.3.0 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add hybrid search (FTS5 + vector kNN + RRF) and embedding pipeline to clawmem, with a DB driver abstraction that auto-upgrades from CLI to better-sqlite3 when available.
...
Run: `cd /Users/oleksiinikitin/Documents/Repos/clawmem && node test/run.js`
 ...
 *   const driver = createDriver('./clawmem.db');
...
function createBetterSqliteDriver(dbPath) {
  ...
  if (sqliteVec) {
    try {
      ...
      console.error(`[clawmem:driver] sqlite-vec load failed: ${err.message}`);
    }
  }
  ...
  function read(sql, params) {
    try {
      ...
      console.error(`[clawmem:driver:sqlite] read error: ${err.message}`);
      return [];
    }
  }
  ...
  function write(sql, params) {
    try {
      ...
      try {
        ...
        console.error(`[clawmem:driver:sqlite] write error: ${err2.message}`);
        return { changes: 0 };
      }
    }
  }
...
Run: `cd /Users/oleksiinikitin/Documents/Repos/clawmem && npm install better-sqlite3 sqlite-vec && node test/run.js`
Expected: All tests PASS including new driver tests

- [ ] **Step 7: Commit**

```bash
cd /Users/oleksiinikitin/Documents/Repos/clawmem
...
- Pass `driver` to `clawmem.extract(adapter, driver, cfg, ...)` instead of `cfg.memoryDbPath`
- Pass `driver` to `clawmem.query.getStats(driver)`, `searchFacts(driver, ...)`, etc.
- Use `dbExists(cfg.memoryDbPath)` instead of `db.exists(...)`
- Close driver at end

(Full file contents are mechanical — every `db.exists(cfg.memoryDbPath)` becomes `dbExists(cfg.memoryDbPath)`, every `clawmem.query.X(cfg.memoryDbPath, ...)` becomes `clawmem.query.X(driver, ...)`.)
...
function getMeta(driver, key) {
  const rows = driver.read(`SELECT value FROM clawmem_meta WHERE key='${esc(key)}'`);
  return rows[0]?.value || null;
}

function setMeta(driver, key, value) {
  driver.write(`INSERT OR REPLACE INTO clawmem_meta (key, value, updated_at) VALUES ('${esc(key)}', '${esc(String(value))}', datetime('now'))`);
}
...
function checkModelConsistency(driver, config) {
  ...
  if (storedModel && storedModel !== config.model) {
    return {
      ok: false,
      error: `Embedding model changed: stored="${storedModel}", config="${config.model}". Run \`clawmem embed --rebuild\` to re-embed with the new model.`,
    };
  }
  return { ok: true };
}
...
async function backfill(driver, config, options = {}) {
  ...
  if (rebuild) {
    ...
    // Clear meta
    driver.write("DELETE FROM clawmem_meta WHERE key LIKE 'embedding_%'");
  }
...
async function search(driver, query, options = {}) {
  ...
  if (!ftsOnly && driver.capabilities.vectors && embeddingConfig) {
    try {
      ...
      console.error(`[clawmem:search] Vector search failed, falling back to FTS5: ${err.message}`);
    }
  }
...
- Add `CLAWMEM_EMBEDDING_API_KEY` and `CLAWMEM_EMBEDDING_BASE_URL` to help text
...
**Files:**
- Modify: `examples/clawmem.json`
- Modify: `examples/clawmem-gemini.json`
- Modify: `examples/clawmem-ollama.json`
- Modify: `examples/clawmem-openclaw.json`
- Create: `examples/clawmem-mixed.json`
- Create: `examples/openclaw-plugin.ts`

- [ ] **Step 1: Update existing example configs**

Add `"embedding"` block (disabled by default) to `examples/clawmem.json`. Add enabled embedding blocks to gemini, ollama, and openclaw examples with provider-appropriate models.

`examples/clawmem-mixed.json`:
```json
{
...
```json
{
  ...
  "license": "MIT",
  "repository": { "type": "git", "url": "https://github.com/pandore/clawmem" },
  "engines": { "node": ">=18.0.0" }
}
```
...
- Update schema table to include vec0 tables and clawmem_meta
...
git commit -m "release: clawmem v0.3.0 — hybrid search, embedding pipeline, driver abstraction"
```
